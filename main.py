...
def init_db():
    ...
    # v0.1.7: tier column for the premium/free toggle. Defaults to 'free'
    # for everyone; premium upgrades flip this to 'premium'. The alert
    # dispatcher reads user.tier when deciding whether to fire alerts for
    # premium-tier hazard events.
    c.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS tier TEXT NOT NULL DEFAULT 'free'")
    ...
    # v0.1.7: toggle architecture columns. severity_class separates opportunity
    # signals (incoming swell, ideal water clarity) from warning signals
    # (advisories, hurricanes) so the same backend can paint the same storm
    # event amber for a coastal resident skin and cyan for a surfer skin.
    # hazard_category is a flat controlled vocabulary used by the deployment-
    # level toggle: EW shows everything, WW filters to water/surf/tsunami.
    # tier_level gates premium-only signals (custom thresholds, faster cron):
    # spatial_join_and_alert checks user.tier >= event.tier_level before firing.
    c.execute("ALTER TABLE ew_events ADD COLUMN IF NOT EXISTS severity_class TEXT")
    c.execute("ALTER TABLE ew_events ADD COLUMN IF NOT EXISTS hazard_category TEXT")
    c.execute("ALTER TABLE ew_events ADD COLUMN IF NOT EXISTS tier_level TEXT NOT NULL DEFAULT 'free'")
    c.execute("CREATE INDEX IF NOT EXISTS idx_ew_events_category ON ew_events (hazard_category)")
    ...
    # === Water Watch foundation (v0.1.7) =====================================
    # ocean_sites: seed reference table for monitoring locations. Populated
    # by a one-shot ETL (seed_ocean_sites.py), not auto-fetched at startup
    # so we don't hammer external sources every boot. About 100 Hawaii DOH
    # beach monitoring stations plus 20 NOAA NDBC / PacIOOS Hawaii buoys
    # to start. UNIQUE(source, source_id) keeps re-runs idempotent.
    c.execute("""CREATE TABLE IF NOT EXISTS ocean_sites (
        id SERIAL PRIMARY KEY,
        source TEXT NOT NULL,
        ...
        default_radius_mi DOUBLE PRECISION NOT NULL DEFAULT 5.0,
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name='ocean_sites' AND column_name='geom'
        ) THEN
            ALTER TABLE ocean_sites
            ADD COLUMN geom geography(Point, 4326)
            GENERATED ALWAYS AS (ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography) STORED;
        END IF;
    END $$;""")
    c.execute("CREATE INDEX IF NOT EXISTS idx_ocean_sites_source ON ocean_sites (source)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_ocean_sites_type ON ocean_sites (site_type)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_ocean_sites_geom ON ocean_sites USING GIST (geom)")

    # buoy_readings: rolling sensor stream from NDBC/PacIOOS. Drawer reads
    # the most-recent row per site for current-conditions display.
    # Threshold-rule cron compares new readings against prior to fire surf
    # or tsunami events into ew_events. Not part of ew_events directly
    # because most readings are boring -- only threshold crossings become
    # events.
    c.execute("""CREATE TABLE IF NOT EXISTS buoy_readings (
        id SERIAL PRIMARY KEY,
        ocean_site_id INTEGER NOT NULL,
        recorded_at TIMESTAMP NOT NULL,
        wave_height_ft DOUBLE PRECISION,
        wave_period_sec DOUBLE PRECISION,
        ...
        raw_payload JSONB,
        fetched_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        UNIQUE (ocean_site_id, recorded_at),
        FOREIGN KEY (ocean_site_id) REFERENCES ocean_sites (id) ON DELETE CASCADE
    )""")
    c.execute("CREATE INDEX IF NOT EXISTS idx_buoy_readings_site_time ON buoy_readings (ocean_site_id, recorded_at DESC)")
    # =========================================================================
